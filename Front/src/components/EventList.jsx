...
const EventList = () => {
  ...
  const [filteredEvents, setFilteredEvents] = useState([]);//Stocke uniquement les événements correspondant au critère de recherche.
  const [searchTerm, setSearchTerm] = useState('');//Enregistre la valeur du champ de recherche saisie par l'utilisateur.
  ...
  const fetchEvents = async () => {
    ...
    // Vérifie si les données reçues sont un tableau (liste d'événements).
      if (response.data && Array.isArray(response.data)) {
        setEvents(response.data);// Enregistre les données (événements) dans l'état events.
        setFilteredEvents(response.data); //Met à jour la liste filtrée avec la même liste pour l’instant.
      } else {
 ...
 //Exécute la fonction fetchEvents une seule fois quand le composant est affiché sur la page.
  const handleSearch = () => {
    const results = events.filter((event) =>
      event.title.toLowerCase().includes(searchTerm.toLowerCase())
    );
    //includes : Vérifie si le titre de l'événement contient le texte recherché.
